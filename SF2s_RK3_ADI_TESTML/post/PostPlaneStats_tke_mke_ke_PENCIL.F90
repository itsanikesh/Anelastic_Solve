...
write(*,*)"CALCULATED TILL HERE"
 if (tecplot_output) then
  ...
  ss2='z'//trim( Sname(stat) )
  write(ss3,'(a,a,i5.5,a4)') trim(outputDIR),trim(SName(stat))//"_",n,".dat"
  open(unit=108,file=ss3,position='append',form='formatted',iostat=s1)
  if(stat.eq.5.or.stat.eq.10) then
  do k = 1,514
   write(108,'(4(4x,e15.8))') time, zc(k), tecvar(k), N2(k)
  enddo
  else
  do k = 1,514
   write(108,'(3(4x,e15.8))') time, zc(k), tecvar(k)
  enddo
  endif
  close(108)

!  call open_tec_binary(time,nzp2,1,ss1,ss2,ss3,'./',0,0)
!  call write_tec_binary(nzp2,1,ztec,0)
!  if (single) then 
!   call write_tec_binary(nzp2,1,tecvar,0)
!  else
!   call write_tec_binary(nzp2,1,tecvar,1)
!  endif
