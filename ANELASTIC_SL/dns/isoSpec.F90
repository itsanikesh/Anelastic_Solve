...
#ifdef PARALLEL 
 use dd,             only: myid, coords, comm3d, rankx1x2, rankX3, rankx1X3, rankx2x3,& 
                           sizex3, cmplxtype,inttype, MPI_ANY_SOURCE, MPI_STATUS_SIZE
#endif
 implicit none
...
#ifdef PARALLEL
 integer              :: sendto, status1(MPI_STATUS_SIZE), tsize
#endif
 stat2=0
 stat=0
...
WsPlane=0.d0
 ...
 do k=0,nzp2-1
#ifdef PARALLEL
  if (myid.EQ.0) then !START MASTER NODE (generate all random numbers on master node
#endif
   do j=0,nyp2-1
    do i=1,nxp2/2-1
...
#ifdef PARALLEL
  endif !END MASTER NODE ONLY
  call MPI_BARRIER(comm3d,stat)
  tsize=size(Usplane)

  if (k.GE.sz.AND.k.LE.ez.AND.rankx1x2.EQ.0.AND.myid.NE.0) call MPI_SEND(myid,1,inttype,0,11,comm3d,stat)
  if (myid.EQ.0.AND.k.GT.ez) then
   call MPI_RECV(sendto,1,inttype,MPI_ANY_SOURCE,11,comm3d,status1,stat)
  else
   sendto=0
  endif
 ...
 if (myid.EQ.0.AND.sendto.NE.0) then
   call MPI_SEND(UsPlane,tsize,cmplxtype,sendto,12,comm3d,stat)
   call MPI_SEND(VsPlane,tsize,cmplxtype,sendto,13,comm3d,stat)
   call MPI_SEND(WsPlane,tsize,cmplxtype,sendto,14,comm3d,stat)
 endif
  if (k.GE.sz.AND.k.LE.ez.AND.rankx1x2.EQ.0.AND.myid.NE.0) then
   call MPI_RECV(UsPlane,tsize,cmplxtype,0,12,comm3d,status1,stat)
   call MPI_RECV(VsPlane,tsize,cmplxtype,0,13,comm3d,status1,stat)
   call MPI_RECV(WsPlane,tsize,cmplxtype,0,14,comm3d,status1,stat)
  endif

  call MPI_BARRIER(comm3d,stat)

  if (k.GE.sz.AND.k.LE.ez.AND.rankx1x2.EQ.0) then
   call Distribute2dM_C(uhat,UsPlane,nxp2/2+1,nyp2,3,k,0,stat)
  endif
  if (k.GE.sz.AND.k.LE.ez.AND.rankx1x2.NE.0) call Distribute2dS_C(uhat,3,k,0,stat)
  call MPI_BARRIER(comm3d,stat)

  if (k.GE.sz.AND.k.LE.ez.AND.rankx1x2.EQ.0) then
   call Distribute2dM_C(vhat,VsPlane,nxp2/2+1,nyp2,3,k,0,stat)
  endif
  if (k.GE.sz.AND.k.LE.ez.AND.rankx1x2.NE.0) call Distribute2dS_C(vhat,3,k,0,stat)
  call MPI_BARRIER(comm3d,stat)

  if (k.GE.sz.AND.k.LE.ez.AND.rankx1x2.EQ.0) then
   call Distribute2dM_C(what,WsPlane,nxp2/2+1,nyp2,3,k,0,stat)
  endif
  if (k.GE.sz.AND.k.LE.ez.AND.rankx1x2.NE.0) call Distribute2dS_C(what,3,k,0,stat)
  call MPI_BARRIER(comm3d,stat)
#else
...
#ifdef PARALLEL
 allocate( PLANE_IN(0:nxp2-1,0:nyp2-1) )
 allocate( PLANE_OUT(0:nxp2/2,0:nyp2-1))
 allocate( PENCIL_IN(0:nzp2-1)  )
 allocate( PENCIL_OUT(0:nzp2-1) )
 call dfftw_plan_dft_1d(plan1dF,nzp2,PENCIL_IN,PENCIL_OUT,FFTW_FORWARD,FFTW_ESTIMATE)
 call dfftw_plan_dft_1d(plan1dB,nzp2,PENCIL_OUT,PENCIL_IN,FFTW_BACKWARD,FFTW_ESTIMATE)
 call dfftw_plan_dft_r2c_2d(plan2dF,nxp2,nyp2,PLANE_IN,PLANE_OUT,FFTW_ESTIMATE)
 call dfftw_plan_dft_c2r_2d(plan2dB,nxp2,nyp2,PLANE_OUT,PLANE_IN,FFTW_ESTIMATE)
#else
...
#ifdef PARALLEL
 use dd,     only: rankx1x2,rankx3, comm3d
#endif
...
!PLANES
 do k=sz-1,ez+1
  !Gather a real x1-x2 plane to x1x2 master 
  if (rankx1x2.EQ.0) call gather2dM(varP,Zplane,nxp2,nyp2,3,k,0,stat)
  if (rankx1x2.NE.0) call gather2dS(varP,3,k,0,stat)
  call MPI_BARRIER(comm3d,stat)

  !On x1x2 master ui(x,y,z) --> ui*(kx,ky,z)
  if (rankx1x2.EQ.0) then 
   PLANE_IN = ZPlane 
   call dfftw_execute(plan2dF)
   PLANE_OUT=PLANE_OUT/dble(nxp2*nyp2)
   ZsPlane = PLANE_OUT
  endif
  call MPI_BARRIER(comm3d,stat)

  !Distribute ui*(kx,ky,z)
  if (rankx1x2.EQ.0) call distribute2dM_C(varS,ZsPlane,nxp2/2+1,nyp2,3,k,0,stat)
  if (rankx1x2.NE.0) call distribute2dS_C(varS,3,k,0,stat)
  call MPI_BARRIER(comm3d,stat)
...
!PENCILS
 do j=sy-1,ey+1
  do i=sx-1,cex+1
   !Gather a complex x3 pencil to x3 master 
   if ( rankx3.EQ.0 ) call gather1dM_C(varS,XYsPencil,nzp2,3,i,j,0,stat)
   if ( rankx3.NE.0 ) call gather1dS_C(varS,3,i,j,0,stat)
   call MPI_BARRIER(comm3d,stat)

   !On x3 master ui*(kx,ky,z)-->ui^(kx,ky,kz)
   if (rankx3.EQ.0) then 
    PENCIL_IN = XYsPencil 
    call dfftw_execute(plan1dF)
    PENCIL_OUT=PENCIL_OUT/dble(nzp2)
    XYsPencil = PENCIL_OUT
   endif
   call MPI_BARRIER(comm3d,stat)

   !Distribute ui^(kx,ky,kz)
   if (rankx3.EQ.0) call distribute1dM_C(varS,XYsPencil,nzp2,3,i,j,0,stat)
   if (rankx3.NE.0) call distribute1dS_C(varS,3,i,j,0,stat)
   call MPI_BARRIER(comm3d,stat)
  enddo
 enddo
#else
...
!PENCILS
 do j=sy-1,ey+1
  do i=sx-1,cex+1
   ...
   !On x3 master ui^(kx,ky,kz)-->ui*(kx,ky,z)
   if (rankx3.EQ.0) then 
    PENCIL_OUT = XYsPencil 
    call dfftw_execute(plan1dB)
    XYsPencil = PENCIL_IN
   endif
   call MPI_BARRIER(comm3d,stat)
...
!PLANES
 do k=sz-1,ez+1
  !Gather a complex x1-x2 plane to x1x2 master 
  if (rankx1x2.EQ.0) call gather2dM_C(varS,ZsPlane,nxp2/2+1,nyp2,3,k,0,stat)
  if (rankx1x2.NE.0) call gather2dS_C(varS,3,k,0,stat)
  call MPI_BARRIER(comm3d,stat)

  !On x1x2 master ui*(kx,ky,z) --> ui(x,y,z)
  if (rankx1x2.EQ.0) then 
   PLANE_OUT = ZsPlane 
   call dfftw_execute(plan2dB)
   ZPlane = PLANE_IN
  endif
  call MPI_BARRIER(comm3d,stat)

  !Distribute ui*(kx,ky,z)
  if (rankx1x2.EQ.0) call distribute2dM(varP,ZPlane,nxp2,nyp2,3,k,0,stat)
  if (rankx1x2.NE.0) call distribute2dS(varP,3,k,0,stat)
  call MPI_BARRIER(comm3d,stat)
...
!@q
 ...
 use grid,   only: zc
#ifdef PARALLEL
 use dd,     only: comm3d, rankx1x2, rankx1x3, rankx2x3, myid
#endif
 implicit none
...
!Local Variables
 ...
 i=sx+10
#ifdef PARALLEL
 if (rankx1x2.EQ.0) call gather2dM(u,Zplane,nxp2,nyp2,3,k,0,stat)
 if (rankx1x2.NE.0) call gather2dS(u,3,k,0,stat)
 call MPI_BARRIER(comm3d,stat)

 if (rankx1x3.EQ.0) call gather2dM(w,Yplane,nxp2,nzp2,2,j,0,stat)
 if (rankx1x3.NE.0) call gather2dS(w,2,j,0,stat)
 call MPI_BARRIER(comm3d,stat)

 if (rankx2x3.EQ.0) call gather2dM(v,Xplane,nyp2,nzp2,1,i,0,stat)
 if (rankx2x3.NE.0) call gather2dS(v,1,i,0,stat)
 call MPI_BARRIER(comm3d,stat)
...
#ifdef PARALLEL
 use dd,         only: realtype, myid, comm3d, MPI_SUM, rankx1x2, rankx3, sizex3, commx3
#endif
 implicit none
...
! Nspec=4*N
...
#ifdef PARALLEL
  !Gather a complex x1-x2 plane to x1x2 master 
  if (rankx1x2.EQ.0) call gather2dM_C(uhat,UsPlane,nxp2/2+1,nyp2,3,k,0,stat)
  if (rankx1x2.NE.0) call gather2dS_C(uhat,3,k,0,stat)
  call MPI_BARRIER(comm3d,stat)

  if (rankx1x2.EQ.0) call gather2dM_C(vhat,VsPlane,nxp2/2+1,nyp2,3,k,0,stat)
  if (rankx1x2.NE.0) call gather2dS_C(vhat,3,k,0,stat)
  call MPI_BARRIER(comm3d,stat)

  if (rankx1x2.EQ.0) call gather2dM_C(what,WsPlane,nxp2/2+1,nyp2,3,k,0,stat)
  if (rankx1x2.NE.0) call gather2dS_C(what,3,k,0,stat)
  call MPI_BARRIER(comm3d,stat)
 ...
 if (rankx3.EQ.0.AND.rankx1x2.EQ.0) then
#else
 k=1
 UsPlane(:,:)=uhat(:,:,k)
 VsPlane(:,:)=vhat(:,:,k)
 ...
 if (rankx3.EQ.sizex3-1 .AND. rankx1x2.EQ.0) then
#else
 k=nzp2
 UsPlane(:,:)=uhat(:,:,k)
 VsPlane(:,:)=vhat(:,:,k)
...
#ifdef PARALLEL
 call MPI_REDUCE(Energy,Etemp,Nspec+1,realtype,MPI_SUM,0,commx3,ierr)
 call MPI_BARRIER(comm3d,stat)
 if (myid.EQ.0) Energy=Etemp
 call MPI_BARRIER(comm3d,stat)


 call MPI_REDUCE(Energy11,Etemp,Nspec+1,realtype,MPI_SUM,0,commx3,ierr)
 call MPI_BARRIER(comm3d,stat)
 if (myid.EQ.0) Energy11=Etemp
 call MPI_BARRIER(comm3d,stat)

 call MPI_REDUCE(Energy22,Etemp,Nspec+1,realtype,MPI_SUM,0,commx3,ierr)
 call MPI_BARRIER(comm3d,stat)
 if (myid.EQ.0) Energy22=Etemp
 call MPI_BARRIER(comm3d,stat)

 call MPI_REDUCE(Energy33,Etemp,Nspec+1,realtype,MPI_SUM,0,commx3,ierr)
 call MPI_BARRIER(comm3d,stat)
 if (myid.EQ.0) Energy33=Etemp
 call MPI_BARRIER(comm3d,stat)

 call MPI_REDUCE(Energy12,Etemp,Nspec+1,realtype,MPI_SUM,0,commx3,ierr)
 call MPI_BARRIER(comm3d,stat)
 if (myid.EQ.0) Energy12=Etemp
 call MPI_BARRIER(comm3d,stat)

 call MPI_REDUCE(Energy23,Etemp,Nspec+1,realtype,MPI_SUM,0,commx3,ierr)
 call MPI_BARRIER(comm3d,stat)
 if (myid.EQ.0) Energy23=Etemp
 call MPI_BARRIER(comm3d,stat)

 call MPI_REDUCE(Energy31,Etemp,Nspec+1,realtype,MPI_SUM,0,commx3,ierr)
 call MPI_BARRIER(comm3d,stat)
 if (myid.EQ.0) Energy31=Etemp
 call MPI_BARRIER(comm3d,stat)

 call MPI_REDUCE(Diss,Etemp,Nspec+1,realtype,MPI_SUM,0,commx3,ierr)
 call MPI_BARRIER(comm3d,stat)
 if (myid.EQ.0) Diss=Etemp
 call MPI_BARRIER(comm3d,stat)
#endif
