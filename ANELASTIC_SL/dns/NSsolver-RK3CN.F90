 ...
 select case(Vmodel)
  ...
  case DEFAULT
   ...
   if (time.GT.50000.d0.and.mod(nstep,25).EQ.0) then
    call zero_temps
    EDIFFtol = 5.d0*rRe*rPr
    call filterHF(rho,r_tmp2,1,1,1,1,'rho',kappaT,EDIFFtol,err1)
    EDIFFtol = 5.d0*rRe*rSc
    call filterHF(scal1,scal1_tmp2,1,1,1,1,'scal1',nappaT,EDIFFtol,err1)
    rho=r_tmp2
    scal1=scal1_tmp2
...
!Viscous Implicit
 ...
 !Corrector steps: project pressure gradient 
  ...
  if (RK3CN) then
   ...
   call Pgrad(Ftemp,u1_tmp2,u2_tmp2,u3_tmp2,dt_rk)
   call ghost(u1_tmp2,'utemp',err1)
   call ghost(u2_tmp2,'vtemp',err1)
   call ghost(u3_tmp2,'wtemp',err1)
  endif
  ...
  elseif (RK3CN) then
    do k=sz,ez
     do j=sy,ey
      do i=sx,ex-EU
       !-dP/!dx
!       Utemp(i,j,k) = Utemp(i,j,k) - ( pn(i+1,j,k) - pn(i,j,k) )*rdxe(i)
      enddo 
     enddo
    enddo
  endif
   ...
   elseif (RK3CN) then
    do k=sz,ez 
     do j=sy,ey-EV
      do i=sx,ex
     !-dP/dy
!      Vtemp(i,j,k) = Vtemp(i,j,k) - ( pn(i,j+1,k) - pn(i,j,k) )*rdye(j)
      enddo
     enddo
    enddo
   endif
   ...
   elseif (RK3CN) then
    do k=sz,ez-EW
     do j=sy,ey
      do i=sx,ex
      !-dP/dz
!       Wtemp(i,j,k) = Wtemp(i,j,k) - ( pn(i,j,k+1) - pn(i,j,k) )*rdze(k) 
      enddo
     enddo
    enddo
   endif
...
!Local Variables
 ...
 do k=sz,ez
  do j=sy,ey
   do i=sx,ex-EU
    Utemp(i,j,k)=Utemp(i,j,k) - ( pn(i+1,j,k)-pn(i,j,k) )*rdxe(i)*dt
   enddo
  enddo
 enddo
 ...
 do k=sz,ez
  do j=sy,ey-EV
   do i=sx,ex
    Vtemp(i,j,k)=Vtemp(i,j,k) - ( pn(i,j+1,k)-pn(i,j,k) )*rdye(j)*dt
   enddo
  enddo
 enddo
 ...
 do k=sz,ez-EW
  do j=sy,ey
   do i=sx,ex
    Wtemp(i,j,k)=Wtemp(i,j,k) - ( pn(i,j,k+1)-pn(i,j,k) )*rdze(k)*dt
   enddo
  enddo
 enddo
...
!     Fortran 90
 ...
 do k=sz,ez
  do j=sy,ey
   do i=sx,ex
      !Calculate d(du_i/dx_i)/dt and add it as a correction to d(H_i)/dx_i so that new field is divergence free
      SRC(i,j,k) =  SRC(i,j,k) + ( (Utemp(i,j,k) - Utemp(i-1,j,k))*rdxc(i) & 
                     +(Vtemp(i,j,k) - Vtemp(i,j-1,k))*rdyc(j) & 
                     +(Wtemp(i,j,k) - Wtemp(i,j,k-1))*rdzc(k) )*rdt
   enddo
  enddo
 enddo
...
!     Fortran 90
 ...
 use ntypes, only: r8
 use Flow,   only: Vmodel, Rmodel, Smodel, p
 use LESmod
 use Grid
 use DD,     only: comm3d,sizeX3,coords
 use Domain, only: sx,ex,sy,ey,sz,ez
     ...
     if (Vmodel.EQ.'DNS'.or.Vmodel.EQ.'SSM'.or.Vmodel.EQ.'DSM'.or.Vmodel.EQ.'DMM') then
      rhs(i,j,k)=f1*veltemp2(i,j,k)+f2*veltemp1(i,j,k)*f3 &
            + rkdt*0.5d0*rRe*( (veltemp2(i,j,k+1) - veltemp2(i,j,k) )*rdzc(k+1)  &
                              -(veltemp2(i,j,k) - veltemp2(i,j,k-1) )*rdzc(k) )*rdze(k)
      !predicted pressure
      rhs(i,j,k) = rhs(i,j,k) - rkdt*( p(i,j,k+1)-p(i,j,k) )*rdze(k) 
...
if (sizeX3.EQ.1) then
  do j=sy-1,ey+1
   do i=sx-1,ex+1
    call Thomas(sub(i,j,:),diag(i,j,:),sup(i,j,:),rhs(i,j,:),ez-sz+2)
   enddo
  enddo
else 
