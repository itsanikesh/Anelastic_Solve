program spectra_1D_kx
 ...
 real(8)                                :: Pi
 integer                                :: x3loc,iter,nspec,nl,nu
 ...
 open(unit=100,file=fname,status='old',form='unformatted',iostat=s1)
  ...
  allocate( SpecStore(0:n1/2,2:n2-1) )
...
!*********************************************************************************
  ...
  do i=0,n1-1
   ...
   if (i.GT.n1/2) then
    ...
    Espec(ishell) = Espec(ishell)&
                   +  dble( dconjg( SpecStore(iconj,n) )*SpecStore(iconj,n) )
   else
    Espec(ishell) = Espec(ishell) &
                   +  dble( dconjg( SpecStore(i,n) )*SpecStore(i,n) )
   endif
  enddo
 enddo
 !**********************************************
 ...
 write(13,*) "# Time:",time
 do i=0,n1/2 
  write(13,'(2(1x,e15.8))') dble(i),Espec(i)/dble((n2-1)-2+1)
 enddo
