...
subroutine mg_restr(lC,lF,phiC,rhsC,cofc1,cofc2,cofc3,cofc4,cofc5,cofc6,cofc7,phiF,srcF,resF)
!@t
...
!Passed Variables
 ...
 real(r8),intent(in)       :: srcF(sxk(lF)-1:exk(lF)+1,syk(lF)-1:&
                                      eyk(lF)+1,szk(lF)-1:ezk(lF)+1)
 real(r8),intent(in)       :: cofc1(sxk(lF)-1:exk(lF)+1,syk(lF)-1:&
                                      eyk(lF)+1,szk(lF)-1:ezk(lF)+1)
 real(r8),intent(in)       :: cofc2(sxk(lF)-1:exk(lF)+1,syk(lF)-1:&
                                      eyk(lF)+1,szk(lF)-1:ezk(lF)+1)
 real(r8),intent(in)       :: cofc3(sxk(lF)-1:exk(lF)+1,syk(lF)-1:&
                                      eyk(lF)+1,szk(lF)-1:ezk(lF)+1)
 real(r8),intent(in)       :: cofc4(sxk(lF)-1:exk(lF)+1,syk(lF)-1:&
                                      eyk(lF)+1,szk(lF)-1:ezk(lF)+1)
 real(r8),intent(in)       :: cofc5(sxk(lF)-1:exk(lF)+1,syk(lF)-1:&
                                      eyk(lF)+1,szk(lF)-1:ezk(lF)+1)
 real(r8),intent(in)       :: cofc6(sxk(lF)-1:exk(lF)+1,syk(lF)-1:&
                                      eyk(lF)+1,szk(lF)-1:ezk(lF)+1)
 real(r8),intent(in)       :: cofc7(sxk(lF)-1:exk(lF)+1,syk(lF)-1:&
                                      eyk(lF)+1,szk(lF)-1:ezk(lF)+1)
...
!Calculate residual
 ...
 do k=szf,ezf
  do j=syf,eyf
   do i=sxf,exf
!      pcfsum =-(cofF(i-1,j,k)+cofF(i+1,j,k) &
!              + cofF(i,j-1,k)+cofF(i,j+1,k) &
!              + cofF(i,j,k-1)+cofF(i,j,k+1))

      resF(i,j,k)=srcF(i,j,k) -( cofc1(i,j,k)*phif(i-1,j,k)    &
                              +  cofc2(i,j,k)*phif(i+1,j,k)    &
                              +  cofc3(i,j,k)*phif(i,j-1,k)    &
                              +  cofc4(i,j,k)*phif(i,j+1,k)    &
                              +  cofc5(i,j,k)*phif(i,j,k-1)    &
                              +  cofc6(i,j,k)*phif(i,j,k+1)    &
                              +  cofc7(i,j,k)*phif(i,j,k) )
