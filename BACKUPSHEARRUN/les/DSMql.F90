...
!Local variables
 ...
 if (debug) call check_point('DSMql#0',.false.)
 ...
 do ij=1,3
  if (ij == 1) then
   do k = sz,ez
    do j = sy,ey
     do i = sx,ex
      tmp1 = 0.5d0 * (ql(i,j,k)+ql(i-1,j,k))
      tmp2 = 0.5d0 * (ql(i,j,k)+ql(i+1,j,k))
   ...
   do k = sz,ez
    do j = sy,ey
     do i = sx,ex
      tmp1 = 0.5d0 * (ql(i,j,k)+ql(i,j-1,k))
      tmp2 = 0.5d0 * (ql(i,j,k)+ql(i,j+1,k))
  ...
  elseif (ij == 3) then
   do k = sz,ez
    do j = sy,ey
     do i = sx,ex
      tmp1 = 0.5d0 * (ql(i,j,k)+ql(i,j,k-1))
      tmp2 = 0.5d0 * (ql(i,j,k)+ql(i,j,k+1))
...
!lestmp2 now stores full MRi
 if (debug) call check_point('DSMql#1',.false.)
...
!Now begin computing LRi
  if (ij == 1) then
   do k = sz,ez
    do j = sy,ey
     do i = sx,ex
      lestmp1(i,j,k) = ql(i,j,k) * 0.5d0 * ( u(i-1,j,k)+u(i,j,k) )
     enddo
    enddo
   enddo
  else if (ij == 2) then
   do k = sz,ez
    do j = sy,ey
     do i = sx,ex
      lestmp1(i,j,k) = ql(i,j,k) * 0.5d0 * ( v(i,j-1,k)+v(i,j,k) )
     enddo
    enddo
   enddo
  else if (ij == 3) then
   do k = sz,ez
    do j = sy,ey
     do i = sx,ex
      lestmp1(i,j,k) = ql(i,j,k) * 0.5d0 * ( w(i,j,k-1)+w(i,j,k) )
     enddo
    enddo
   enddo
  endif
...
!lestmp3 now stores filter(product) of LRi
  ...
  if (ij == 1) then
   ...
   call filter(ql,lestmp,2,xfil,yfil,zfil,'cfluc',err1)
...
!Compute numerator LRi*MRi, stored in CTsgs
  Cqlsgs=Cqlsgs+lestmp2*lestmp3
 enddo !ij loops
 call ghost(Cqlsgs,'cfluc',err1)
 call ghost(lestmp4,'cfluc',err1)

 if (debug) call check_point('DSMql#2',.false.)

!Average in homogenous direction: x,y for VSL
  if (C_avg_dir.EQ.12) then
   call avgX1X2(Cqlsgs,tmpZd1,'cfluc')
   call avgX1X2(lestmp4,tmpZd2,'cfluc')
   do k=sz,ez
    do j=sy,ey
     do i=sx,ex
      Cqlsgs(i,j,k) = tmpZd1(k)
      lestmp4(i,j,k) = tmpZd2(k)
      if (Cqlsgs(i,j,k).LT.0.d0) Cqlsgs(i,j,k) = 0.d0
     enddo
    enddo
   enddo
  else if (C_avg_dir.EQ.2) then
   call avgX2(Cqlsgs,tmpXZd1,tmpXZd3,'cfluc')
   call avgX2(lestmp4,tmpXZd2,tmpXZd3,'cfluc')
   do k=sz,ez
    do j=sy,ey
     do i=sx,ex
      Cqlsgs(i,j,k) = tmpXZd1(i,k)
   ...
   write(IOUT,'(a)')"ABORTING DSMql: C_avg_dir is not valid!", C_avg_dir
   stop
  end if
 call ghost(Cqlsgs,'cfluc',err1)
 call ghost(lestmp4,'cfluc',err1)
  ...
  do j = sy,ey
   do i = sx,ex
    if (lestmp4(i,j,k)==0.d0) then
     Cqlsgs(i,j,k) = 1.d-20 !prevent divide by zero 
    else
     Cqlsgs(i,j,k) = 0.5d0 * Cqlsgs(i,j,k) / lestmp4(i,j,k)
    endif
   enddo
  enddo
 enddo
 call ghost(Cqlsgs,'cfluc',err1)
 call filterHF(Cqlsgs,lestmp4,2,1,1,1,'cfluc',kappaql,1.d0,err1)
 Cqlsgs=lestmp4
 call ghost(Cqlsgs,'cfluc',err1)

 if (debug) call check_point('DSMql#4',.false.)

 call MaxMin(Cqlsgs,tmp1,tmp2,err1)

 write(IOUT,*)"Cqlsgs: max =", tmp1, " min = ", tmp2

return
end subroutine DSMql
