...
!Local variables
 ...
 real(r8):: tmp1, tmp2, tmp3, del_g, nappa_sgs
 ...
 do ij=1,3
  if (ij == 1) then
   do k = sz,ez
    do j = sy,ey
     do i = sx,ex
      tmp1 = 0.5d0 * (scal1(i,j,k)+scal1(i-1,j,k))
      tmp2 = 0.5d0 * (scal1(i,j,k)+scal1(i+1,j,k))
   ...
   do k = sz,ez
    do j = sy,ey
     do i = sx,ex
      tmp1 = 0.5d0 * (scal1(i,j,k)+scal1(i,j-1,k))
      tmp2 = 0.5d0 * (scal1(i,j,k)+scal1(i,j+1,k))
  ...
  elseif (ij == 3) then
   do k = sz,ez
    do j = sy,ey
     do i = sx,ex
      tmp1 = 0.5d0 * (scal1(i,j,k)+scal1(i,j,k-1))
      tmp2 = 0.5d0 * (scal1(i,j,k)+scal1(i,j,k+1))
...
!Now begin computing LRi
  if (ij == 1) then
   do k = sz,ez
    do j = sy,ey
     do i = sx,ex
      lestmp1(i,j,k) = scal1(i,j,k) * 0.5d0 * ( u(i-1,j,k)+u(i,j,k) )
     enddo
    enddo
   enddo
  else if (ij == 2) then
   do k = sz,ez
    do j = sy,ey
     do i = sx,ex
      lestmp1(i,j,k) = scal1(i,j,k) * 0.5d0 * ( v(i,j-1,k)+v(i,j,k) )
     enddo
    enddo
   enddo
  else if (ij == 3) then
   do k = sz,ez
    do j = sy,ey
     do i = sx,ex
      lestmp1(i,j,k) = scal1(i,j,k) * 0.5d0 * ( w(i,j,k-1)+w(i,j,k) )
     enddo
    enddo
   enddo
  endif
...
!lestmp3 now stores filter(product) of LRi
  ...
  if (ij == 1) then
   ...
   call filter(scal1,lestmp,2,xfil,yfil,zfil,'cfluc',err1)
...
!Average in homogenous direction: x,y for VSL
  if (C_avg_dir.EQ.12) then
   ...
   write(IOUT,'(a)')"ABORTING DSMscal1: C_avg_dir is not valid!", C_avg_dir
   stop
  end if
 call ghost(CSsgs,'cfluc',err1)
 call ghost(lestmp4,'cfluc',err1)
  ...
  do j = sy,ey
   do i = sx,ex
    if (lestmp4(i,j,k)==0.d0) then
     CSsgs(i,j,k) = 1.d-20 !prevent divide by zero 
    else
     CSsgs(i,j,k) = 0.5d0 * CSsgs(i,j,k) / lestmp4(i,j,k)
    endif
   enddo
  enddo
 enddo
 call ghost(CSsgs,'cfluc',err1)
 call filterHF(CSsgs,lestmp4,2,1,1,1,'cfluc',nappaT,1.d0,err1)
 CSsgs=lestmp4
 call ghost(CSsgs,'cfluc',err1)

 if (debug) call check_point('DSMscal1#4',.false.)

 call MaxMin(CSsgs,tmp1,tmp2,err1)
 write(IOUT,*)"CSsgs: max =", tmp1, " min = ", tmp2

return
end subroutine DSMscal1
