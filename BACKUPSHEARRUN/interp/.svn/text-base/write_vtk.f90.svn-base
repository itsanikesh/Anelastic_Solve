subroutine write_vtk_field(filename,title,nx,ny,nz,x,y,z,var)
!@t
! \textbf{subroutine write\_vtk\_field(filename,title,nx,ny,nz,x,y,z,var)}
!@h
!   Description:
!     Write a .vtk file for use with paraview and matlab. 
!@q
!   Current Code Owner:
...
!Passed Variables
 ...
 real(8),intent(in)          :: x(1:nx),y(1:ny),z(1:nz)
 real(8),intent(in)          :: var(1:nx,1:ny,1:nz)
...
!Open VTK File
 open(13,file=trim(filename),access='stream',form='unformatted',status='new', &
