...
!  l=6
 ...
 case('psource')
  l=7
 case('utemp','vtemp','wtemp','rtemp','s1temp','qtemp')
  l=7
...
!@q
 ...
 use boundc,  only: BND
#ifdef PARALLEL
 use dd,      only: sizeX1, sizeX2, sizeX3
#endif
  implicit none
 ...
 select case(vtype)
  ...
  case('wtemp')
   ...
   of2=1
   iW=EW
  case('c','p','psource','fluc','rho','rp','rf','uc','vc','wc','rtemp','qtemp','cfluc','scal1','scal1temp','s1temp','qv','ql','qr')
   dxt=>dxe
   dyt=>dye
   dzt=>dze
   xt=>xc
...
!X1-Direction
 if (dir.EQ.1) then  !0
...
#ifdef PARALLEL
    if(sizeX1.NE.1) then
     call parallel_periodic_BC(var,face,dir,iU)
     else
#endif
      var(sx-1,:,:) = var(ex-iU,:,:) 
      var(ex+1-iU,:,:) = var(sx,:,:)
   ...
   elseif (Btype.EQ.8) then !1
    ...
    write(IOUT,'(a16,i2,a24,i2)') "ERROR: BC TYPE: ", Btype, " is not (1,2,3) for dir, ",dir, "vtype", vtype 
   endif !1
...
#ifdef PARALLEL
    if(sizeX2.NE.1) then
     call parallel_periodic_BC(var,face,dir,iV)
    else
#endif
      var(:,sy-1,:) = var(:,ey-iV,:) 
      var(:,ey+1-iV,:) = var(:,sy,:)
...
#ifdef PARALLEL
    if(sizeX3.NE.1) then
     call parallel_periodic_BC(var,face,dir,iW)
    else
#endif
      var(:,:,sz-1) = var(:,:,ez-iW) 
      var(:,:,ez+1-iW) = var(:,:,sz)  
...
#ifdef PARALLEL
subroutine parallel_periodic_BC(var,face,dir,ivec)
!@t
! \textbf{subroutine parallel\_periodic\_BC(var,face,dir,ivec)}
!@h
!   Description:
!     Enforces the periodic boundary condition for the parallel version.
!@q
!   Current Code Owner:
!     Matt de Stadler (mdestadl@ucsd.edu)
...
!@h
!   Comments:
!     This subroutine only exists for the parallel version. MPI is used
!     to transfer data between the ghost cells used to enforce periodicity.
!     KYLE CAN YOU ADD A FEW MORE DETAILS ON WHAT THIS DOES / HOW TO USE IT???
!@q

 use ntypes, only: r8
 use IO,     only: IOUT
 use dd,     only: commx1,commx2,commx3,sizex1,sizex2,sizex3,rankx1, &
                   rankx2,rankx3,realtype, MPI_STATUS_SIZE
 use Domain, only: sx,ex,sy,ey,sz,ez
 implicit none

!Passed Variables
 real(r8),intent(inout)    :: var(sx-1:ex+1,sy-1:ey+1,sz-1:ez+1)
 integer,intent(in)        :: dir,face,ivec

!Local Variables
 integer                                :: i,j,k,l,m,n
 real(r8),allocatable,dimension(:,:)    :: splane, rplane
 integer                                :: tsize, s1
 ...
 if (dir.EQ.1) then !0
  !Allocate temporary transfer arrays
  allocate( splane(sy-1:ey+1,sz-1:ez+1), STAT=s1 )
  allocate( rplane(sy-1:ey+1,sz-1:ez+1), STAT=s1 )
  !Set transfer size
  tsize=size(splane)
  !Pack plane, exchange planes between start and end nodes, Unpack plane
   if (rankx1.EQ.0) then
    splane(:,:) = var(sx,:,:)
    call mpi_sendrecv(splane,tsize,realtype,sizex1-1,1,rplane,tsize,realtype,sizex1-1,2,commx1,Status2,ierr)
    var(sx-1,:,:) = rplane(:,:)
   endif
   if (rankx1.EQ.sizex1-1) then 
    splane(:,:) = var(ex-ivec,:,:)
    call mpi_sendrecv(splane,tsize,realtype,0,2,rplane,tsize,realtype,0,1,commx1,Status2,ierr)
    var(ex+1-ivec,:,:) = rplane(:,:)
   endif
 elseif (dir.EQ.2) then !0
  allocate( splane(sx-1:ex+1,sz-1:ez+1), STAT=s1 )
  allocate( rplane(sx-1:ex+1,sz-1:ez+1), STAT=s1 )
  tsize=size(splane)
  !Exchange planes between start and end nodes
  !Pack plane, exchange planes between start and end nodes, Unpack plane
   if (rankx2.EQ.0) then
    splane(:,:) = var(:,sy,:)
    call mpi_sendrecv(splane,tsize,realtype,sizex2-1,1,rplane,tsize,realtype,sizex2-1,2,commx2,Status2,ierr)
    var(:,sy-1,:) = rplane(:,:)
   endif
   if (rankx2.EQ.sizex2-1) then 
    splane(:,:) = var(:,ey-ivec,:)
    call mpi_sendrecv(splane,tsize,realtype,0,2,rplane,tsize,realtype,0,1,commx2,Status2,ierr)
    var(:,ey+1-ivec,:) = rplane(:,:)
   endif
 elseif (dir.EQ.3) then !0
  allocate( splane(sx-1:ex+1,sy-1:ey+1), STAT=s1 )
  allocate( rplane(sx-1:ex+1,sy-1:ey+1), STAT=s1 )
  tsize=size(splane)
   if (rankx3.EQ.0) then
    splane(:,:) = var(:,:,sz)
    call mpi_sendrecv(splane,tsize,realtype,sizex3-1,1,rplane,tsize,realtype,sizex3-1,2,commx3,Status2,ierr)
    var(:,:,sz-1) = rplane(:,:)
   endif
   if (rankx3.EQ.sizex3-1) then 
    splane(:,:) = var(:,:,ez-ivec)
    call mpi_sendrecv(splane,tsize,realtype,0,2,rplane,tsize,realtype,0,1,commx1,Status2,ierr)
    var(:,:,ez+1-ivec) = rplane(:,:)
   endif
 else !0
  write(IOUT,*) "ERROR IN PERIODIC BC:", dir, "is not a valid direction (1,2,3)"
  endif !0

 deallocate(splane, STAT=s1)
 deallocate(rplane, STAT=s1)
 
return
end subroutine parallel_periodic_BC 
#endif
...
!*******************************************************************************
 ...
 case('Read','read')  !READ X1 BC's
  block='X1min'
   ...
   !QV
   call scaninichar(infile,block,'QVTYPE',inBC,defval,IOUTL)
   select case(inBC)
    case('Dirichlet','dirichlet','dir')
     TB(1,1,9)=1
    case('Neumann','neumann','neu')
   ...
   call scaninireal(infile,block,'QVVALUE',VB(1,1,9),defval,IOUTL)

   !QL
   call scaninichar(infile,block,'QLTYPE',inBC,defval,IOUTL)
   select case(inBC)
    case('Dirichlet','dirichlet','dir')
     TB(1,1,10)=1
    case('Neumann','neumann','neu')
   ...
   call scaninireal(infile,block,'QLVALUE',VB(1,1,10),defval,IOUTL)

   !QR
   call scaninichar(infile,block,'QRTYPE',inBC,defval,IOUTL)
   select case(inBC)
    case('Dirichlet','dirichlet','dir')
     TB(1,1,11)=1
    case('Neumann','neumann','neu')
   ...
   call scaninireal(infile,block,'QRVALUE',VB(1,1,11),defval,IOUTL)
  ...
  block='X1max'
   ...
   call scaninireal(infile,block,'QVVALUE',VB(1,2,9),defval,IOUTL)
   ...
   call scaninireal(infile,block,'QLVALUE',VB(1,2,10),defval,IOUTL)
   ...
   call scaninireal(infile,block,'QRVALUE',VB(1,2,11),defval,IOUTL)
...
!*******************************************************************************
 ...
 case('Read','read')  !READ X2 BC's
  block='X2min'
   ...
   call scaninireal(infile,block,'QVVALUE',VB(2,1,9),defval,IOUTL)
   ...
   call scaninireal(infile,block,'QLVALUE',VB(2,1,10),defval,IOUTL)
   ...
   call scaninireal(infile,block,'QRVALUE',VB(2,1,11),defval,IOUTL)
  ...
  block='X2max'
   ...
   call scaninireal(infile,block,'QVVALUE',VB(2,2,9),defval,IOUTL)
   ...
   call scaninireal(infile,block,'QLVALUE',VB(2,2,10),defval,IOUTL)
   ...
   call scaninireal(infile,block,'QRVALUE',VB(2,2,11),defval,IOUTL)
...
!*******************************************************************************
 ...
 case('Read','read')  !READ X3 BC's
  ...
  !U3
   ...
   call scaninireal(infile,block,'QVVALUE',VB(3,1,9),defval,IOUTL)
   ...
   call scaninireal(infile,block,'QLVALUE',VB(3,1,10),defval,IOUTL)
   ...
   call scaninireal(infile,block,'QRVALUE',VB(3,1,11),defval,IOUTL)
  ...
  block='X3max'
   ...
   call scaninireal(infile,block,'QVVALUE',VB(3,2,9),defval,IOUTL)
   ...
   call scaninireal(infile,block,'QLVALUE',VB(3,2,10),defval,IOUTL)
   ...
   call scaninireal(infile,block,'QRVALUE',VB(3,2,11),defval,IOUTL)
