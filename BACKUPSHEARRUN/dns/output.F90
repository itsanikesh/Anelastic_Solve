...
!@q
 ...
 use ntypes, only: r8
 use Flow,   only: u,v,w,p,rho,scal1,qv,ql,qr,u1_tmp2,u2_tmp2,u3_tmp2,&
 ...
 if (Vmodel.EQ.'DNS'.or.Vmodel.EQ.'SSM'.or.Vmodel.EQ.'DSM'.or.Vmodel.EQ.'DMM') then
   ...
   !qv
    call write_plane(qv,2,jplanes(j),0,0,'qv',ver,err1)
   !ql
    call write_plane(ql,2,jplanes(j),0,0,'ql',ver,err1)
   !qr
    call write_plane(qr,2,jplanes(j),0,0,'qr',ver,err1)
   enddo
   ...
   !qv
    call write_plane(qv,3,kplanes(k),0,0,'qv',ver,err1)
   !qr
    call write_plane(qr,3,kplanes(k),0,0,'qr',ver,err1)
   !ql
    call write_plane(ql,3,kplanes(k),0,0,'ql',ver,err1)
   enddo
