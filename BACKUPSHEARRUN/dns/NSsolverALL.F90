...
!Local Variables
 ...
 logical,parameter                     :: debug=.false.
 integer(i4)                           :: i,j,k
 real(r8)                              :: rcond(sx-1:ex+1,sy-1:ey+1,sz-1:ez+1)
 real(r8)                              :: rho_iflag0(sx-1:ex+1,sy-1:ey+1,sz-1:ez+1)
 real(r8)                              :: qv_iflag0(sx-1:ex+1,sy-1:ey+1,sz-1:ez+1)
 real(r8)                              :: ql_iflag0(sx-1:ex+1,sy-1:ey+1,sz-1:ez+1)
 real(r8)                              :: qsat, Exner, delta_q, iflag, Latv, c
 ...
 call rho_rhs(u,v,w,rho,rcond,Ftemp,iflag,err1)
 ...
 !Water Vapor
 call qv_rhs(u,v,w,qv,rcond,Ftemp,iflag,err1)
 call var_advance(Ftemp,q_tmp1,a1,a2,dt)
  if (debug) call write_plane(r_tmp2,1,nxp2/2,0,1,'NSr',.false.,err1)
  if (debug) call check_point('rk_ss#8',.false.)

 !Cloud/Liquid Water
! call ql_rhs(u,v,w,ql,rcond,Ftemp,iflag,err1)
...
!  if (debug) call check_point('rk_ss#8',.false.)
 ...
 call ghost(q_tmp1,'rtemp',err1)
 call ghost(q_tmp2,'rtemp',err1)
...
!Viscous Implicit
 ...
 if (RK3CN) call var_advance_CN_PL(r_tmp2,rho_iflag0,1.d0,a3,1.d0,dt_rk,5)
 if (RK3CN) call var_advance_CN_PL(q_tmp1,qv_iflag0,1.d0,a3,1.d0,dt_rk,9)
! if (RK3CN) call var_advance_CN_PL(q_tmp2,ql_iflag0,1.d0,a3,1.d0,dt_rk,10)
 ...
 do k=sz-1,ez+1
  do j=sy-1,ey+1
   do i=sx-1,ex+1
    ...
    at(i,j,k) = rho_iflag0(i,j,k)*Exner
    Latv = 2.5*(10.0)**6.0 - (1885.0-4186.0)*(at(i,j,k)-273.15)
    qsat = 3.80/(pzz(i,j,k)*0.01)*exp(17.27*(at(i,j,k)-273.0)/(at(i,j,k)-36.0))
    if(qv_iflag0(i,j,k).gt.qsat.or.qv_iflag0(i,j,k).eq.qsat) then
     delta_q = (qv_iflag0(i,j,k)-qsat)/(1.0+(qsat*Latv**2.0)/(1004.0*461.0*at(i,j,k)**2.0))
     rcond(i,j,k) = delta_q/delt
    elseif(qv_iflag0(i,j,k).lt.qsat) then
     rcond(i,j,k) = 0.0
    endif
!    if(myid.eq.10)write(*,*), rcond(i,j,k),i,j,k
   enddo
  enddo
 enddo
 write(*,*), maxval(rcond(:,:,:)),maxval(r_tmp2(:,:,:)),maxval(at(:,:,:)),maxval(rho_iflag0(:,:,:)),myid
 STOP
 iflag = 1.0
 !Potential Temperature/Passive Scalar
 call rho_rhs(u,v,w,rho,rcond,Ftemp,iflag,err1)
 call var_advance(Ftemp,r_tmp2,a1,a2,dt)
  if (debug) call write_plane(r_tmp2,1,nxp2/2,0,1,'NSr',.false.,err1)
  if (debug) call check_point('rk_ss#8',.false.)
...
!Rain Water
! call qr_rhs(u,v,w,qr,Ftemp,iflag,err1)
...
!  if (debug) call check_point('rk_ss#8',.false.)
 ...
 call ghost(q_tmp1,'rtemp',err1)
 call ghost(q_tmp2,'rtemp',err1)
 call ghost(q_tmp3,'rtemp',err1)

 if (RK3CN) call var_advance_CN_PL(r_tmp2,rho,1.d0,a3,1.d0,dt_rk,5)
 if (RK3CN) call var_advance_CN_PL(q_tmp1,qv,1.d0,a3,1.d0,dt_rk,9)
! if (RK3CN) call var_advance_CN_PL(q_tmp2,ql,1.d0,a3,1.d0,dt_rk,10)
 ...
 do k=sz-1,ez+1
  do j=sy-1,ey+1
   do i=sx-1,ex+1
    ...
    Latv = 2.5*(10.0)**6.0 - (1885.0-4186.0)*(at(i,j,k)-273.15)
    qsat = 3.80/(pzz(i,j,k)*0.01)*exp(17.27*(at(i,j,k)-273.0)/(at(i,j,k)-36))
...
!     Fortran 90
 ...
 use ratios,     only: r_1_2
 use Flow,       only: densityBG,ALPHA,ALPHA_0,at,qv,ql,Vmodel,r
...
subroutine rho_rhs(un,vn,wn,rhon,rcond,Rtemp,iflag,stat)
!@t
 ...
 !*********************************
  ...
  !********************************* 
   do k=sz,ez
    do j=sy,ey
     do i=sx,ex
!      if(iflag.eq.0) pzz(i,j,k) = pz(i,j,k)+p(i,j,k)
       Exner = (pzz(i,j,k)/101325.0)**(287.0/1004.0)
        Latv = 2.5*(10.0)**6.0 - (1885.0-4186.0)*(at(i,j,k)-273.15)
...
subroutine qv_rhs(un,vn,wn,rhon,rcond,Rtemp,iflag,stat)
!@t
! \textbf{subroutine rho\_rhs(un,vn,wn,rhon,Rtemp,stat)}
!@h
!   Description:
...
subroutine ql_rhs(un,vn,wn,rhon,rcond,Rtemp,iflag,stat)
!@t
! \textbf{subroutine rho\_rhs(un,vn,wn,rhon,Rtemp,stat)}
!@h
!   Description:
...
subroutine qr_rhs(un,vn,wn,rhon,Rtemp,iflag,stat)
!@t
! \textbf{subroutine rho\_rhs(un,vn,wn,rhon,Rtemp,stat)}
!@h
!   Description:
...
!Extra-diffusion
   ...
   if (coords(3).EQ.sizeX3-1) then
    if (var.EQ.1.or.var.EQ.2.or.var.EQ.3) then
     ...
     enddo
     elseif (var.EQ.5.or.var.EQ.8) then
     do k = sz-1, ez+1
