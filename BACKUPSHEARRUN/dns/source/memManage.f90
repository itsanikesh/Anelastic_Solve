...
!FLOW VARIABLE ALLOCATION
 ...
 allocate( rcond(sx-1:ex+1,sy-1:ey+1,sz-1:ez+1), stat=s1 )
 if (s1.NE.0) then
  write(IOUT,*) "Error Allocating at"
  goto 1000
 endif
 ...
 allocate( iflag0_rho(sx-1:ex+1,sy-1:ey+1,sz-1:ez+1), stat=s1 )
 if (s1.NE.0) then
  write(IOUT,*) "Error Allocating ALPHA_0"
  goto 1000
 endif

 allocate( iflag0_qv(sx-1:ex+1,sy-1:ey+1,sz-1:ez+1), stat=s1 )
 if (s1.NE.0) then
  write(IOUT,*) "Error Allocating ALPHA_0"
  goto 1000
 endif

 allocate( iflag0_ql(sx-1:ex+1,sy-1:ey+1,sz-1:ez+1), stat=s1 )
 if (s1.NE.0) then
  write(IOUT,*) "Error Allocating ALPHA_0"
  goto 1000
 endif
...
!LES VARIABLE ALLOCATION
  ...
  allocate( Cqvsgs(sx-1:ex+1,sy-1:ey+1,sz-1:ez+1), stat=s1 )
  if (s1.NE.0) then
   write(IOUT,*) "Error Allocating LES CTsgs"
   goto 1000
  endif

  allocate( Cqlsgs(sx-1:ex+1,sy-1:ey+1,sz-1:ez+1), stat=s1 )
  if (s1.NE.0) then
   write(IOUT,*) "Error Allocating LES CTsgs"
   goto 1000
  endif

  allocate( Cqrsgs(sx-1:ex+1,sy-1:ey+1,sz-1:ez+1), stat=s1 )
  if (s1.NE.0) then
   write(IOUT,*) "Error Allocating LES CTsgs"
   goto 1000
  endif
  ...
  allocate( kappaqv(sx-1:ex+1,sy-1:ey+1,sz-1:ez+1), stat=s1 )
  if (s1.NE.0) then
   write(IOUT,*) "Error Allocating LES kappaT"
   goto 1000
  endif

  allocate( kappaql(sx-1:ex+1,sy-1:ey+1,sz-1:ez+1), stat=s1 )
  if (s1.NE.0) then
   write(IOUT,*) "Error Allocating LES kappaT"
   goto 1000
  endif

  allocate( kappaqr(sx-1:ex+1,sy-1:ey+1,sz-1:ez+1), stat=s1 )
  if (s1.NE.0) then
   write(IOUT,*) "Error Allocating LES kappaT"
   goto 1000
  endif
