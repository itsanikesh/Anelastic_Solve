 ...
 select case (Smodel) 
  ...
  if (time.GT.0.d0.and.mod(nstep,50).EQ.0) then
   if (Rmodel.EQ.'SSMrho'.or.Rmodel.EQ.'DSMrho') then
    call zero_temps
    EDIFFtol = -0.d0*rRe*rPr
    call filterHF(qv,q_tmp2,2,1,1,1,'rho',kappaqv,EDIFFtol,err1)
    !call filter(rho,r_tmp2,2,1,1,1,'rho',err1)
    qv=q_tmp2
    call ghost(qv,'qv',err1)
    write(IOUT,*)"Filtering qv in XYZ dirs: special FIL!"
   endif
  endif
  ...
  if (time.GT.0.d0.and.mod(nstep,50).EQ.0) then
   if (Rmodel.EQ.'SSMrho'.or.Rmodel.EQ.'DSMrho') then
    call zero_temps
    EDIFFtol = -0.d0*rRe*rPr
    call filterHF(ql,q_tmp1,2,1,1,1,'rho',kappaql,EDIFFtol,err1)
    !call filter(rho,r_tmp2,2,1,1,1,'rho',err1)
    ql=q_tmp1
    call ghost(ql,'ql',err1)
    write(IOUT,*)"Filtering ql in XYZ dirs: special FIL!"
   endif
  endif
  ...
  if (time.GT.0.d0.and.mod(nstep,50).EQ.0) then
   if (Rmodel.EQ.'SSMrho'.or.Rmodel.EQ.'DSMrho') then
    call zero_temps
    EDIFFtol = -0.d0*rRe*rPr
    call filterHF(qr,q_tmp3,2,1,1,1,'rho',kappaqr,EDIFFtol,err1)
    !call filter(rho,r_tmp2,2,1,1,1,'rho',err1)
    qr=q_tmp3
    call ghost(qr,'qr',err1)
    write(IOUT,*)"Filtering qr in XYZ dirs: special FIL!"
   endif
  endif
...
!Viscous Implicit
 ...
 do k=sz-1,ez+1
  do j=sy-1,ey+1
   do i=sx-1,ex+1
    if(ql(i,j,k).lt.0.0) ql(i,j,k) = 0.0
...
! write(*,*),"ACTUAL RHO",maxval(rho(:,:,:)) 
 do k=sz-1,ez+1
  do j=sy-1,ey+1
   do i=sx-1,ex+1
    if(qr(i,j,k).lt.0.0) qr(i,j,k) = 0.0
    if(ql(i,j,k).lt.0.0) ql(i,j,k) = 0.0
    Exner = (pzz(i,j,k)/101325.0)**(287.0/1004.0)
...
!     write(*,*),ar(i,j,k),cr(i,j,k),er(i,j,k),vrr(i,j,k),i,j,k
!    else
!     ar(i,j,k) = 0.0
!     cr(i,j,k) = 0.0
!     er(i,j,k) = 0.0
...
!     Fortran 90
 ...
 use Parameters, only: rRe, rPr, Rsponge, flow_type, time, RK3, RK3CN, LESflag
 use Flow,   only: Rmodel,r
 use LESmod, only: kappaqv, kappasm
 use IO, only: IOUT
 use DD,  only: comm3d,sizeX3,coords
 implicit none
  ...
  !*********************************
   do k=sz,ez
    do j=sy,ey
     do i=sx,ex
      !-dQ1/dx
      kappaTaver2 = 0.5d0*(kappaqv(i+1,j,k)+kappaqv(i,j,k))
      kappaTaver1 = 0.5d0*(kappaqv(i,j,k)+kappaqv(i-1,j,k))
      Rtemp(i,j,k) = Rtemp(i,j,k) - LESflag*1.d0*( kappaTaver2*(rhon(i+1,j,k)-rhon(i,j,k))*rdxe(i)&
                                          -kappaTaver1*(rhon(i,j,k)-rhon(i-1,j,k))*rdxe(i-1) )*rdxc(i)
      !-dQ2/dy 
      kappaTaver2 = 0.5d0*(kappaqv(i,j+1,k)+kappaqv(i,j,k))
      kappaTaver1 = 0.5d0*(kappaqv(i,j,k)+kappaqv(i,j-1,k))
      Rtemp(i,j,k) = Rtemp(i,j,k) - LESflag*1.d0*( kappaTaver2*(rhon(i,j+1,k)-rhon(i,j,k))*rdye(j)&
                                          -kappaTaver1*(rhon(i,j,k)-rhon(i,j-1,k))*rdye(j-1) )*rdyc(j)
      !-dQ3/dz: march with CN  
      !kappaTaver2 = 0.5d0*(kappaT(i,j,k+1)+kappaT(i,j,k))
   ...
   if (RK3) then
    do k=sz,ez
     do j=sy,ey
      do i=sx,ex
      !-dQ3/dz: march with CN  
       kappaTaver2 = 0.5d0*(kappaqv(i,j,k+1)+kappaqv(i,j,k))
       kappaTaver1 = 0.5d0*(kappaqv(i,j,k)+kappaqv(i,j,k-1))
       Rtemp(i,j,k) = Rtemp(i,j,k) - LESflag*1.d0*( kappaTaver2*(rhon(i,j,k+1)-rhon(i,j,k))*rdze(k)&
                                           -kappaTaver1*(rhon(i,j,k)-rhon(i,j,k-1))*rdze(k-1) )*rdzc(k)
      enddo
     enddo
...
!     Fortran 90
 ...
 use Parameters, only: rRe, rPr, Rsponge, flow_type, time, RK3, RK3CN, LESflag
 use Flow, only: Rmodel, r
 use LESmod, only: kappaql, kappasm
 use IO, only: IOUT
 use DD,  only: comm3d,sizeX3,coords
 implicit none
  ...
  !*********************************
   do k=sz,ez
    do j=sy,ey
     do i=sx,ex
      !-dQ1/dx
      kappaTaver2 = 0.5d0*(kappaql(i+1,j,k)+kappaql(i,j,k))
      kappaTaver1 = 0.5d0*(kappaql(i,j,k)+kappaql(i-1,j,k))
      Rtemp(i,j,k) = Rtemp(i,j,k) - LESflag*1.d0*( kappaTaver2*(rhon(i+1,j,k)-rhon(i,j,k))*rdxe(i)&
                                          -kappaTaver1*(rhon(i,j,k)-rhon(i-1,j,k))*rdxe(i-1) )*rdxc(i)
      !-dQ2/dy 
      kappaTaver2 = 0.5d0*(kappaql(i,j+1,k)+kappaql(i,j,k))
      kappaTaver1 = 0.5d0*(kappaql(i,j,k)+kappaql(i,j-1,k))
      Rtemp(i,j,k) = Rtemp(i,j,k) - LESflag*1.d0*( kappaTaver2*(rhon(i,j+1,k)-rhon(i,j,k))*rdye(j)&
                                          -kappaTaver1*(rhon(i,j,k)-rhon(i,j-1,k))*rdye(j-1) )*rdyc(j)
      !-dQ3/dz: march with CN  
      !kappaTaver2 = 0.5d0*(kappaT(i,j,k+1)+kappaT(i,j,k))
   ...
   if (RK3) then
    do k=sz,ez
     do j=sy,ey
      do i=sx,ex
      !-dQ3/dz: march with CN  
       kappaTaver2 = 0.5d0*(kappaql(i,j,k+1)+kappaql(i,j,k))
       kappaTaver1 = 0.5d0*(kappaql(i,j,k)+kappaql(i,j,k-1))
       Rtemp(i,j,k) = Rtemp(i,j,k) - LESflag*1.d0*( kappaTaver2*(rhon(i,j,k+1)-rhon(i,j,k))*rdze(k)&
                                           -kappaTaver1*(rhon(i,j,k)-rhon(i,j,k-1))*rdze(k-1) )*rdzc(k)
      enddo
     enddo
...
!     Fortran 90
 ...
 use Parameters, only: rRe, rPr, Rsponge, flow_type, time, RK3, RK3CN, LESflag
 use Flow, only: Rmodel,r
 use LESmod, only: kappaqr, kappasm
 use IO, only: IOUT
 use DD,  only: comm3d,sizeX3,coords
 implicit none
  ...
  !*********************************
   do k=sz,ez
    do j=sy,ey
     do i=sx,ex
      !-dQ1/dx
      kappaTaver2 = 0.5d0*(kappaqr(i+1,j,k)+kappaqr(i,j,k))
      kappaTaver1 = 0.5d0*(kappaqr(i,j,k)+kappaqr(i-1,j,k))
      Rtemp(i,j,k) = Rtemp(i,j,k) - LESflag*1.d0*( kappaTaver2*(rhon(i+1,j,k)-rhon(i,j,k))*rdxe(i)&
                                          -kappaTaver1*(rhon(i,j,k)-rhon(i-1,j,k))*rdxe(i-1) )*rdxc(i)
      !-dQ2/dy 
      kappaTaver2 = 0.5d0*(kappaqr(i,j+1,k)+kappaqr(i,j,k))
      kappaTaver1 = 0.5d0*(kappaqr(i,j,k)+kappaqr(i,j-1,k))
      Rtemp(i,j,k) = Rtemp(i,j,k) - LESflag*1.d0*( kappaTaver2*(rhon(i,j+1,k)-rhon(i,j,k))*rdye(j)&
                                          -kappaTaver1*(rhon(i,j,k)-rhon(i,j-1,k))*rdye(j-1) )*rdyc(j)
      !-dQ3/dz: march with CN  
      !kappaTaver2 = 0.5d0*(kappaT(i,j,k+1)+kappaT(i,j,k))
   ...
   if (RK3) then
    do k=sz,ez
     do j=sy,ey
      do i=sx,ex
      !-dQ3/dz: march with CN  
       kappaTaver2 = 0.5d0*(kappaqr(i,j,k+1)+kappaqr(i,j,k))
       kappaTaver1 = 0.5d0*(kappaqr(i,j,k)+kappaqr(i,j,k-1))
       Rtemp(i,j,k) = Rtemp(i,j,k) - LESflag*1.d0*( kappaTaver2*(rhon(i,j,k+1)-rhon(i,j,k))*rdze(k)&
                                           -kappaTaver1*(rhon(i,j,k)-rhon(i,j,k-1))*rdze(k-1) )*rdzc(k)
      enddo
     enddo
  ...
  if (Vmodel.EQ.'SSM'.or.Vmodel.EQ.'DSM'.or.Vmodel.EQ.'DMM') then
   do k = sz,ez
    ...
    elseif (var.EQ.9) then
     tmp1 = -0.5d0*( kappaqv(i,j,k)+kappaqv(i,j,k-1) )*raver1/r(i,j,k)
     tmp2 = -0.5d0*( kappaqv(i,j,k)+kappaqv(i,j,k+1) )*raver2/r(i,j,k)
    elseif (var.EQ.10) then
     tmp1 = -0.5d0*( kappaql(i,j,k)+kappaql(i,j,k-1) )*raver1/r(i,j,k)
     tmp2 = -0.5d0*( kappaql(i,j,k)+kappaql(i,j,k+1) )*raver2/r(i,j,k)
    elseif (var.EQ.11) then
     tmp1 = -0.5d0*( kappaqr(i,j,k)+kappaqr(i,j,k-1) )*raver1/r(i,j,k)
     tmp2 = -0.5d0*( kappaqr(i,j,k)+kappaqr(i,j,k+1) )*raver2/r(i,j,k)
    elseif (var.EQ.8) then
   ...
   case (1,2,5,8,9,10,11)
     ...
     if (Vmodel.EQ.'SSM'.or.Vmodel.EQ.'DSM'.or.Vmodel.EQ.'DMM') then
      ...
      elseif (var.EQ.2) then
       ...
       tmp1 = -0.5d0*( kappaqv(i,j,k)+kappaqv(i,j,k-1) )*raver1/r(i,j,k)
       tmp2 = -0.5d0*( kappaqv(i,j,k)+kappaqv(i,j,k+1) )*raver2/r(i,j,k)
      else if (var.EQ.10) then
       tmp1 = -0.5d0*( kappaql(i,j,k)+kappaql(i,j,k-1) )*raver1/r(i,j,k)
       tmp2 = -0.5d0*( kappaql(i,j,k)+kappaql(i,j,k+1) )*raver2/r(i,j,k)
      else if (var.EQ.11) then
       tmp1 = -0.5d0*( kappaqr(i,j,k)+kappaqr(i,j,k-1) )*raver1/r(i,j,k)
       tmp2 = -0.5d0*( kappaqr(i,j,k)+kappaqr(i,j,k+1) )*raver2/r(i,j,k)
