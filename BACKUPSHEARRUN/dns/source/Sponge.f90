...
!@q
 ...
 use ntypes,     only: r8
 use Flow,       only: u,v,w,p,rho,scal1,qv,ql,qr
...
!Local Variables
 ...
 allocate( X1inf(sy-1:ey+1,sz-1:ez+1,1:2,1:11), stat=s1 )
 allocate( X2inf(sx-1:ex+1,sz-1:ez+1,1:2,1:11), stat=s1 )
 allocate( X3inf(sx-1:ex+1,sy-1:ey+1,1:2,1:11), stat=s1 )
