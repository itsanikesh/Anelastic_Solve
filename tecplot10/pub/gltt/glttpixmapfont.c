...
#if defined AMTEC
  pixmaps= new FTPixmapFont(instance, angle);
#else /* AMTEC */
...
//  glAlphaFunc( GL_GEQUAL, 0.1 );
  ...
  for(;;)
    ...
    glBitmap( 0, 0,
              0.0, 0.0,
              GLfloat(double(gpixmap->getDeltaX())/64.0),
              GLfloat(double(gpixmap->getDeltaY())/64.0),
              (const GLubyte *)0 );
    if( data != 0 )
      {
...
#define PI_OVER_180 (3.14159265359 / 180.0)
              GLfloat(double(gpixmap->getAdvance()) / 64.0 * cos(gpixmap->getAngle() * PI_OVER_180)-
                      double(gpixmap->getDeltaX())/64.0),
              GLfloat(double(gpixmap->getAdvance()) / 64.0 * sin(gpixmap->getAngle() * PI_OVER_180)-
                      double(gpixmap->getDeltaY())/64.0),
#else /* AMTEC */
              GLfloat(double(gpixmap->getAdvance()-gpixmap->getDeltaX())/64.0),
              GLfloat(double(gpixmap->getDeltaY())/-64.0),
#endif /* AMTEC */
              (const GLubyte *)0 );
    }
...
#if defined AMTEC
void GLTTPixmapFont::load( int from /* = 0 */, int to /* = 255 */ )
{
  if(pixmaps)
    {
    pixmaps->load(from, to);
    }
}
#endif /* AMTEC */
