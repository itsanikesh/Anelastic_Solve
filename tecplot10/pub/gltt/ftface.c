...
GLTTboolean FTFace::makeIndicesTable()
{
  ...
  if( i == n )
    ...
    short unsigned platform= 0;
    short unsigned encoding= 0;
    TT_Get_CharMap_ID( *face, n - 1, &platform, &encoding );
    TT_Get_CharMap( *face, n - 1, &char_map );
#endif /* AMTEC */

    for( int ascii_code= 0; ascii_code < 256; ++ascii_code )
      {
#if defined AMTEC
      /* Kludge to work with symbol fonts: */
      int glyph_index;
      if(platform == 3 && encoding == 0) /* symbol font. */
        glyph_index = TT_Char_Index( char_map, ascii_code + 0xF000 );
      else
        glyph_index= TT_Char_Index( char_map, CP_Translate(ascii_code) );
...
#else /* AMTEC */
    /*DET
     * This code HAS to be broken. It uses char_map uninitialized!!
     * It also doesn't work with symbol fonts.
     */
