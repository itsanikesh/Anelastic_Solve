                                              ...
                                              char **ErrMsg)             /* OUT (only if returning FALSE) */
{
   ...
   * Example:
   *
   * $!ADDONCOMMAND ADDONID='Get Style Exerciser' COMMAND='MYCOMMAND'
   */
...
/**
 * Controls the scheduling of the UpdateCurrentTabPageOnIdle function.
 */
static Boolean_t UpdateCurrentTabPageOnIdleIsScheduled = FALSE;

/**
 * If scheduled this function is called when Tecplot is idle to update the
 * sidebar.
 */
static void STDCALL UpdateCurrentTabPageOnIdle(ArbParam_t ClientData)
{
  TecUtilLockStart(AddOnID);

  UpdateCurrentTabPage();
  UpdateCurrentTabPageOnIdleIsScheduled = FALSE;
 ...
 */
static void ScheduleUpdateCurrentTabPageOnIdle(void)
{
  /* only schedule the on-idle callback if it isn't already */
  if (!UpdateCurrentTabPageOnIdleIsScheduled)
    {
      UpdateCurrentTabPageOnIdleIsScheduled = TRUE;
      TecUtilOnIdleQueueAddCallback(UpdateCurrentTabPageOnIdle, 0);
    }
}
    ...
    */
    ...
    /* Version 9 and later Note: If you are using modeless dialogs, you should
      ...
      case StateChange_UnsuspendInterface:  /* Replaces StateChange_DrawGraphicsOff */
        {
          /* these items shouldn't effect style properties */
        } break;
      ...
      case StateChange_ZonesAdded:      /* set of added zones */
        {
          ScheduleUpdateCurrentTabPageOnIdle();
        } break;
      default: break;
    } /* end switch */
}
