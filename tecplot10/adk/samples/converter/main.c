 ...
 * processed.
 */
static Boolean_t STDCALL MacroCommandCallback(char  *MacroCommandString,
                                              char **ErrMsg)
{
  ...
  /* 
   * MacroCommandString is the add-on macro command string
   * needing processing.
   *
   * "*ErrMsg" is an error message string which must be allocated
   * and set by this function if and only if the return value is
   * FALSE.
   */
   ...
   * Example:
   *
   * $!ADDONCOMMAND ADDONID='Simple Spreadsheet Converter' COMMAND='MYCOMMAND'
   */
...
EXPORTFROMADDON void STDCALL InitTecAddOn(void)
{
  /*
   * NOTE:
   *   TecUtilLockOn MUST be used for InitTecAddOn instead of
   *   TecUtilLockStart because AddonID has yet to be
