...
so that the master copy can be updated.
-----------------------------------------------------------------------
...
For UNIX, et.al.:
 cc preplot.c -DPLOT3D -DUNIXX -DCRAYT3M  -o preplot        (Cray T3M only)
 cc preplot.c -DPLOT3D -DUNIXX -DOLDCRAY  -o preplot        (Older crays like YMP's)
 cc preplot.c -DPLOT3D -DUNIXX -DOLDCRAY -DWEEDMIDBLOCKS -o preplot
 cc preplot.c -DPLOT3D -DUNIXX -DCRAYIEEE -o preplot
 cc preplot.c -DPLOT3D -DUNIXX -DIRIS -o preplot
 cc preplot.c -DPLOT3D -DUNIXX -DSUN -o preplot
 cc preplot.c -DPLOT3D -DUNIXX -DDEC -o preplot
 cc preplot.c -DPLOT3D -DUNIXX -DHITACHI -o preplot
 cc preplot.c -DPLOT3D -DUNIXX -DDECALPHA -o preplot
 cc preplot.c -DPLOT3D -DUNIXX -DCOMPAQALPHA -o preplot
 cc preplot.c -DPLOT3D -DUNIXX -DHP -o preplot
 cc preplot.c -DPLOT3D -DUNIXX -DLINUX -o preplot
 cc preplot.c -DPLOT3D -DUNIXX -DIBMRS6000 -o preplot
...
For Windows:
 cl /DMSWIN preplot.c (Microsoft C)
...
which case please let me know what you came up with..... 425-653-1200 
(ask for byron) or e-mail byron@tecplot.com)
...
--------------------------------------------------------------
               - Fixed limits on plot3d input files to
                 be able to handel i,j, or k indices
...
--------------------------------------------------------------
               ...
               - Changed zone and var names to be allocated
                 dynamically.  This way more names can be used.
--------------------------------------------------------------
Version: 6.3f                                         07/29/93
--------------------------------------------------------------
               - Fixed PLOT3D i-blank with 3D data core dump (CAM)
               - Added checking for exceeding MaxNumVars and
                 MaxNumZones (CAM)
--------------------------------------------------------------
Version: 6.3g                                         08/20/93
--------------------------------------------------------------
               - Added NV flag (CAM)
               - Added check for malloc == NULL in RecogCust* (CAM)
               - Fixed check for error when E= is missing from
                 FEPOINT or FEBLOCK zones.
               - Fixed problem with memory allocation for strings
                 (DOS ONLY)
--------------------------------------------------------------
Version: 6.3h                                         08/24/93
--------------------------------------------------------------
               - Fixed problem with preplot not checking if enough
                 elements are read in for FE Data.
               - Added check for -ip,-jp,-kp flags to make sure -3dw
                 or -3dp is also used.
               - Added exit (0) to end of main.  Sun workstations
                 return random exit codes if none is supplied.
--------------------------------------------------------------
Version: 6.3i                                         11/23/93
--------------------------------------------------------------
               - Added -function flag for plot3d.  You can now
                 read in PLOT3D "function" files.
               - Changed the way individual lines are read in
                 so abrupt end-of-file's (like emacs generates)
                 can be handled correctly.
               - Added check for PLOT3D files so D exponent
                 format can be used.
               - Added more code for CRAY's to weed out "mid record
                 blocks"  See WEEDMIDBLOCKS compiler option.
               - Added check for limit of custom axis labels.
--------------------------------------------------------------
Version: 6.3j                                         05/25/94
--------------------------------------------------------------
               - Fixed problem with using FEConnect in duplist when
                 zone header fails to specify N= or E=.
--------------------------------------------------------------
Version: 6.3k                                         08/18/94
--------------------------------------------------------------
               - Added CRAYIEEE compiler directive.  Use this to
                 make version that can read IEEE binary Plot3D files
                 on a CRAY (see the cray "assign" command to create
                 IEEE files on a CRAY).
               - Fixes to check for text strings that are too long.
--------------------------------------------------------------
Version: 6.3L                                         01/20/95
--------------------------------------------------------------
               - Added support to allow reading of both a "function"
                 and a "q" file at the same time.  Also now
                 requires ".f" files if a function file is
                 used.
--------------------------------------------------------------
Version: 6.3m                                         03/29/95
--------------------------------------------------------------
               - Initial coding to add V7 stuff.
--------------------------------------------------------------
Version: 7.0-06/23/95                                 06/23/95
--------------------------------------------------------------
BEGIN CODELOG PREPLOT
C 09-30-96
C   - Added USERREC record capability
C   - Change allowable angle for text to
C     be between -360 and 360.
C   - Added fix for reading binary PLOT3D files
C     on a Dec Alpha.
C   - Added code to various sections so preplot
C     recovers more gracefully when error occurs.
V 09-30-96
V  --------------------------------------------------------------
V         Version: 7.0b-09/30/96
V  --------------------------------------------------------------
B 10-04/96
B   Line Number assigned to errors was not initialized
B   correctly.
V 10-04-96
V  --------------------------------------------------------------
V         Version: 7.0c-10/04/96
V  --------------------------------------------------------------
B 10-04-96
B   Files not being closed properly
V 10-04-96
V  --------------------------------------------------------------
V         Version: 7.0d-10/04/96
V  --------------------------------------------------------------
B 01-17-97
B    Fixed problem with using -iset ,0,0,1
B    (i.e. grabbing just the last i value)
V 01/17/97
V  --------------------------------------------------------------
V  Version: 7.0e-01/17/97                                01/17/97
V  --------------------------------------------------------------
C 01/31/97
C    Added code to prevent accidental deletion of
C    the input file.
B 01/31/97
B    Fixed problem of reading in large values that
B    are in integer format.
V 01/31/97
V  --------------------------------------------------------------
V  Version: 7.0f-01/31/97                                01/31/97
V  --------------------------------------------------------------
C 02/05/97
C    Added -collapseijk flag as temporary "patch"
C    to plot3d -3dw JK-Ordered and IK-Ordered zones.
V 02/05/97
V  --------------------------------------------------------------
V  Version: 7.0g-02/05/97                                01/31/97
V  --------------------------------------------------------------
C 04/09/97
C    Changed zone and var names to be limited to
C    63 characters.
V 04/09/97
V  --------------------------------------------------------------
V  Version: 7.0h-04/09/97                                04/09/97
V  --------------------------------------------------------------
C 06/05/97 (CAM)
C    Changed GetRandomFileName and made BlckFName
C    dynamic so TECPLOTKERNEL works better.
B 06/05/97
B    Fix so can compile on the CRAY  (Not done
B    since conversion to V7)
C 06/05/97
C    Added capability to process unstructured
C    plot3d (FAST) grid files.  use -unstructured
V 06/05/97
V  --------------------------------------------------------------
V  Version: 7.0i-06/05/97                                06/05/97
V  --------------------------------------------------------------
V 01/13/98
V  --------------------------------------------------------------
V  Version: 7.5a-01/13/98                                01/13/98
V  --------------------------------------------------------------
C 01/19/98 (DTO)
C   Added "MFC" (macro function command) parameter
C   to text & geom sections.
V 01/19/98
V  --------------------------------------------------------------
V  Version: 7.5b-01/19/98                                01/19/98
V  --------------------------------------------------------------
C 07/08/98 (DTO)
B   Fixed fopen to use "rb" when input is binary
C   Added -foreign to reverse input bytes.
C 07/08/98 (RMS)
C   Used ABS < 1.0 instead of just < 1.0 in the
C   test for old window mode geometries.
C 07/08/98 (CAM)
C   Made it an error to anchor frame coordinates geometries
C   outside of frame.  Previously it was an error.
C 07/08/98 (CAM)
C   Changed error message to use '@' instead of '|' so
C   it shows up better on the screen.
C 07/08/98 (CAM)
C   Added test for anchor position between 0 and 1 in
C   the test for old window mode geometries.
B 07/08/98 (BDP)
B   Fixed a bug such that the -[ijk]set flags
B   settings are no longer applied to FE zones.
B   This has the effect of allowing the user to
B   have a mixed bag of zones, i.e. ordered and
B   FE, while allowing skipping to be applied to
B   those zones for which it makes sense.
V 07/08/98
V  --------------------------------------------------------------
V  Version: 7.5d-07/08/98                                07/08/98
V  --------------------------------------------------------------
C 07/31/98 (CAM)
C   Changed MaxCharsFileName to 260 or _MAXPATH and
C   changed a few places where 150 was hard-coded in
C   for the maximum size.
C 08/02/98 (CAM)
C   Changed MaxNumVars and MaxNumZones to 32700 and
C   MaxChrsVarName and MaxChrsZnTitle to 64 so they
C   match Tecplot.
C 08/11/98 (DTO)
C   Changed SetSize to be MaxNumZones/32 and ZoneSet, etc.
C   to be dynamically allocated.
B 01/21/99 (RMS) <75357>
B   Code now examines error code before using global variables
B   effected by error conditions.
C 09/03/1999 (DTO)
C   Changed MaxGeomPoints to use MaxPtsInALine (if Tecplot) or default
C   to 32000 (if command line).
C 09/07/1999 (RMS)
C   Including "GLOBAL.h" causes problems with preplot and therefore
C   any function needed in Tecplot is simply manually declared in
C   the preplot source. The inclusion of "GLOBAL.h" was removed and
C   a function declaration was added to access the maximum points
C   in a line.
C 09/07/1999 (RMS)
C   MaxChrsTextString was increased from 1025 to 2048.
B 09/08/1999 (BDP) <75487>
B   Fixed problem with preplot (internal to tecplot and as stand-alone).  
B   ASCII Datafiles with "garbage" at the end and no trailing newline would 
B   hang preplot (and thus tecplot).
V 09/08/1999
V  --------------------------------------------------------------
V  Version: 7.5e-09/08/99                                09/08/99
V  --------------------------------------------------------------
B 09/28/1999 (RMS)
B   I forgot to add the bug comment in the last delta so this notice
B   highlights the changes in the last delta. Added a lower bounds check
B   on an array access to prevent under-indexing. This is especially
B   evident when preplot tries to parse a bogus plt file. In addition
B   I updated the FOPEN/FCLOSE and Open_File/Close_File calls.
C 10/13/1999 (RMS)
C   I tightened up the assertions for Open_File and as a result had
C   to modify parameters passed here.
C 10/19/1999 (BDP)
C   Changed function ReadALine() to handle \r\n at end of file more
C   gracefully.
V 10/19/1999
V  --------------------------------------------------------------
V  Version: 7.5f-10/19/99                                10/19/99
V  --------------------------------------------------------------
C 04/21/2000 (BDP)
C   Changed parsing of zonelist parameter to now correctly allow
C   zonenumbers up to DEFAULT_MAXNUMZONES.  Used to be limited to
C   2048.  Setting ISet,KSet etc. are still unnecessarily limited to
C   DEFAULT_MAXNUMZONES however.
B 08/09/2000 (BDP)
B   Fixed problem with repeat factors in plot3d files when they occur
B   in the dimension list in both grid and solution files.
C 09/20/2000 (BDP)
C   Added -stopafterlastzone
V 09/20/2000
V  --------------------------------------------------------------
V  Version: 7.5g-09/20/2000                            09/20/2000
V  --------------------------------------------------------------
B 12/08/2000 (RMS)
B   Fixed problem with reading in data where J and K were specified
B   but I was not.
V  --------------------------------------------------------------
V  NOTE: Version history greatly out of date 09/20/00 to 01/13/02
V  --------------------------------------------------------------
V  --------------------------------------------------------------
V  Version: 10.0-01/14/2002                            01/14/2002
V  --------------------------------------------------------------
V  --------------------------------------------------------------
V  NOTE: Version history greatly out of date 01/14/02 to 01/30/02
V  --------------------------------------------------------------
C 01/30/2003 (CAM)
C   Fixed problem reading in ordered data files that were missing
C   last value.
V  --------------------------------------------------------------
V  Version: PREPLOT V10.1-01/30/2003
V  --------------------------------------------------------------
C 04/14/2003 (CAM)
C   Added new geometry draw order option.
V  --------------------------------------------------------------
V  Version: PREPLOT V10.2-04/14/2003
V  --------------------------------------------------------------
C 09/05/2003 (RMS)
C   Changed MaxLineIndex to 32000 for files with really long lines.
C   This changes was made on 10/28/2003 by (DTO) in version but was
C   not added to version 10 on the same day by mistake.
C 09/05/2003 (CAM)
C   Changed to work with 3d text.
V  --------------------------------------------------------------
V  Version: PREPLOT V10.21-10/16/2003
V  --------------------------------------------------------------
C 03/17/2004 (CAM)
C   Changed for 64-bit Windows compile
V  --------------------------------------------------------------
V  Version: PREPLOT V10.22-03/17/2004
V  --------------------------------------------------------------
C 03/29/2004 (RMS)
C   Changed call to mktemp with one to umask and mkstemp for creating temporary
C   files as recommended by linker on linux platform. Change was made for all
C   Unix/Linux flavors. Windows still uses _mktemp.
B 03/29/2004 (RMS) <2668>
B   Plot3D part of preplot did not get upgraded to output version 100 binary plt
B   files. It was producing version 75 format with a version 100 header. This
B   only effected plot3d files converted with preplot, not the plot3d loader.
V  --------------------------------------------------------------
V  Version: PREPLOT V10.23-03/29/2004
V  --------------------------------------------------------------
B 05/25/2004 (RMS) <2727>
B   Fixed preplot's auto file name extension creation. Previously preplot would
B   short circut and stop after the first period instead of looking for the
B   last.
V  --------------------------------------------------------------
V  Version: PREPLOT V10.24-05/25/2004
V  --------------------------------------------------------------
B 06/01/2004 (RMS) <2727>-revisited
B   Preplot files are specified without extension and therefore do not need to
B   have the extension stripped. The code that was stripping the extension
B   should only have been used for non-plot3d files.
V  --------------------------------------------------------------
V  Version: PREPLOT V10.25-06/01/2004
V  --------------------------------------------------------------
B 06/16/2004 (BDP) <unreported>
B   Fixed problem with processing of 1d and 3d plot3d files.  Preplot
B   was generating the wrong (i.e. old syntax) binary header.
V  --------------------------------------------------------------
V  Version: PREPLOT V10.26-06/16/2004
V  --------------------------------------------------------------
C 07-01-2004 (RMS)
C   Added a new flag to allow preplot to output binary data files with foreign
C   byte order: "-foreignplt".
V  --------------------------------------------------------------
V  Version: PREPLOT V10.27-07/01/2004
V  --------------------------------------------------------------
END CODELOG
*/

#define TecplotBinaryFileVersion 102 /* copied form GLOBAL.h */
#define VersionString "PREPLOT V10.27-07/01/2004  Copyright (C) Tecplot, Inc. 1987-2004.\n"

#define MAXNUMWARNINGS 50


/*
 * Need to define Boolean_t here to use in following includes
 */
...
#include <ctype.h>
#include <string.h>
#define  MaxCharsFileName   (_MAX_PATH+1) /* about 260 */
#endif
...
#             endif
  ...
  if (VarIsCellCentered != NULL)
    {
      FREE_ARRAY(VarIsCellCentered,"VarIsCellCentered");
          ...
          if (Line[LinePos] == '[')
            {
               ...
               * found at any other time it is an error. Note the assumption
               * here is that only sets use recursion. If this changes we will
               * have to use another flag to indicate that we are processing
               * sets instead of just relying on the recursion flag.
               */
              if (RecursionLevel == 1)
                {
                ...
                ErrorMessage("Sets can not be nested.", TRUE);
            }
          else if (Line[LinePos] == ']')
            {
              /*
               * Sets are processed recursively (see '[' above), therefore if
               * an ']' token is found at any other time it is an error.  Note
               * the assumption here is that only sets use recursion. If this
               * changes we will have to use another flag to indicate that we
               * are processing sets instead of just relying on the recursion
               * flag.
               */
              if (RecursionLevel > 1)
                {
              ...
              if (C == '-')
                {
                   ...
                   */
                  if (ParenTokenNestLevel != 0 && I > 1 && !EFound && !DFound)
                    {
  ...
  if (PreplotIsOk)
    {
      if (ZoneType[CurZone] == ORDERED)
        {
          LgIndex I,J,K;
          for (K = 1; (K <= KMax) && PreplotIsOk; K++)
          for (J = 1; (J <= JMax) && PreplotIsOk; J++)
          for (I = 1; (I <= IMax) && PreplotIsOk; I++)
            {
              if (!IsCellCentered ||
                  ((IMax == 1 || I < IMax) &&
                   (JMax == 1 || J < JMax) &&
...
#endif
                          WriteR(F,ScaleFact*CurTValue,DT);
                        }
                      N++;
                    }
  ...
  while ((CurToken == IdentToken) && (PreplotIsOk))
    {
      if (CompareIdent(CurData,"T"))
        ...
        {
          ZoneAuxDataListItem = GetAuxData("AUXDATA", &ZoneAuxDataListItem);
  ...
  if (PreplotIsOk)
    {
      if ((ZoneTypeSet || DataPackingSet) &&
        ...
        {
          ErrorMessage("Can not mix 'ZONETYPE' and 'DATAPACKING' options "
...
static void RecogGeometryHeader(void)
{
  ...
  if (PreplotIsOk &&
      ...
      (RSet || ThetaSet))
    {
      ErrorMessage("Can not mix X,Y, and Z with R and THETA.",TRUE);
    }
                                          ...
                                          for (I = 0; PreplotIsOk && (I < L); I++)
                                            {
                                              ...
                                              if (PreplotIsOk && (NodeVar >= 0))
                                              ...
                                              else if (PreplotIsOk)
                                                {
                                                  ...
                                                  if (ZoneIsEnabled)
                                                    WriteI(BlckFile,ROUND2(CurTValue));
                                                }
                                  ...
                                  while (Connection < FaceNeighborConnections && PreplotIsOk)
                                    {
                                      ...
                                      if (IsOk && FaceNeighborMode == GLOBALONETOONE)
                                        {
                                          NeighborZone = ROUND2(CurTValue) - 1;
                                      ...
                                      if (FaceNeighborMode != LOCALONETOONE && IsOk)
                                        {
                                          ...
                                          if (IsOk && FaceNeighborMode == GLOBALONETOONE)
                                            {
                                              LgIndex NeighborElement = IValue - 1;
                                              ...
                                              if (FaceNeighborMode == LOCALONETOMANY)
                                                ...
                                                for (Assoc = 0;
                                                     ...
                                                     Assoc++)
                                                  {
                                                    ...
                                                    if (IsOk)
                                                      {
                                                        LgIndex NeighborElement = ROUND2(CurTValue) - 1;
                                                        if (NeighborElement == -1 && NeighborZone != -1)
                                                          {
                                                            IsOk = FALSE;
                                                            Message =
 ...
 */
                  ...
                  else if (CurToken == DATASETAUXDATAToken)
                    {
                      ...
                      GetAuxData("DATASETAUXDATA", &DataSetAuxDataListItem);
                      if (PreplotIsOk)
                        {
                      ...
                      if (PreplotIsOk)
                        {
                          ...
                          GetAuxData("VARAUXDATA", &VarAuxDataListItem);
                          if (PreplotIsOk)
                            {
...
I.  The header section.
    ...
    definitions.
     ...
     i.  Magic number, Version number
         +-----------+
         | "#!TDV101"|       8 Bytes, exact characters "#!TDV100".
     ...
     iv.  Zones
         ...
         if FE Zone:
           ...
           +-----------+       
           +-----------+
           | INT32*3   |     ICellDim,JCellDim,KCellDim
           +-----------+       
         ...
         If the above is 1, then supply the following:
           ...
           +-----------+      
           +-----------+
           | INT32     |     Auxiliary Value Format (Currently only allow 0=AuxiliaryValueFormat_String)
           +-----------+
           +-----------+
...
If the geometry type is Ellipse then
    ...
    ix. Dataset Auxiliary data.
         +-----------+
         | FLOAT32   |       DataSetAux Marker;  F=799
         +-----------+
         +-----------+
...
II.  Data section
     ...
     i.  ordered zones
         ...
         if "has variable sharing" != 0
           +-----------+
           | INT32*nvar|     zone number to share variable with (relative to this datafile).
           +-----------+     (-1 = no sharing).   (Omit entirely if "Has variable sharing" is 0).
         +-----------+
         | INT32     |       Zone number to share connectivity list with (-1 = no sharing).
         +-----------+
         +-----------+
     ...
     ii. fem zones
         ...
         if "has variable sharing" != 0
           ...
           +-----------+     (-1 = no sharing).   (Omit entirely if "Has variable sharing" is 0.
         +-----------+
         | INT32     |       Zone number to share connectivity lists with (-1 = no sharing).
         +-----------+
         +-----------+
         | xxxxxxxxxx|       Zone Data.  Each variable is in data format as
         +-----------+       specified above.
...
3.  This represents JMax sets of adjacency indices where each
